...
async function ensureRuntimeTables(): Promise<void> {
  try {
    ...
    // trip_log (migration 0001) also didn't apply on Turso — the impact log
    // insert 500s ("Failed query: insert into trip_log ..."). Recreate it too.
    await libsql.execute(`CREATE TABLE IF NOT EXISTS trip_log (
    ...
    await libsql.execute(
      `CREATE INDEX IF NOT EXISTS trip_log_user_idx ON trip_log (user_id)`,
    );
  } catch (err) {
